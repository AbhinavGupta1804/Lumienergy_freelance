...
Workflow:
  1. Background poller reads Google Sheets every ~20s
  ...
  4. SQLite dedup store prevents repeat calls for the same row
...
from app.services.sheets_poller import SheetsPoller
...
async def lifespan(app: FastAPI):
    """Startup: logging, SQLite dedup, background sheet poller. Shutdown: stop poller."""
    ...
    poller = SheetsPoller(dedup_store)
